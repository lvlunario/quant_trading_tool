...
`python -m atlas --synthetic-csv-demo fixtures/synthetic-broker.csv` exposes this boundary using the checked-in invented fixture. The command hashes the exact input bytes, derives an opaque source ID from that digest, uses the runtime as the synthetic `as_of`, and prints no ticker, quantity or price fields. A blocked reconciliation returns exit code 3. Add `--ledger /absolute/private/path/import-audit.sqlite3` to record a hash-only receipt; an identical retry reports `exact_replay` and zero publishable rows. The runtime timestamp makes the fixture repeatable for interface testing but is not evidence of broker-export freshness. This path has only synthetic fixtures; it does not establish the header names, footer rules, cash semantics or rounding behavior of a Fidelity export.
