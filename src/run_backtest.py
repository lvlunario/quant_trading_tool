...
# --- DEFINITIVE FIX: Configuration now matches the professional training script ---
TICKER = "AAPL" 
QLSTM_MODEL_PATH = "models/saved_models/qlstm_best.pth"
AGENT_SAVE_PATH = "models/saved_models/qa3c_agent.pth"
INITIAL_CAPITAL = 50000
INPUT_DIM = 5 # This MUST match the input dim in the trained model
...
def run_backtest():
    """Runs the full backtest for the trained QA3C agent."""
    ...
    if not os.path.exists(AGENT_SAVE_PATH):
        print("Error: Trained agent not found. Run train_qa3c.py"); return
        
    # --- DEFINITIVE FIX: Initialize agent with the correct input dimension ---
    agent = QA3C(input_dim=INPUT_DIM, action_dim=3)
    agent.load_state_dict(torch.load(AGENT_SAVE_PATH))
    agent.eval()

    print("3. Setting up professional trading environment...")
    env = TradingEnvironment(test_data, QLSTM_MODEL_PATH, initial_capital=INITIAL_CAPITAL)
    ...
    ax.plot(portfolio_history.index, portfolio_history['value'], label='Quantum Agent Strategy', color='royalblue', linewidth=2)
    ax.plot(benchmark_history.index, benchmark_history['value'], label='Buy and Hold Benchmark', color='grey', linestyle='--')
    ax.set_title(f'Quantum Trading Strategy vs. Benchmark for {TICKER}', fontsize=16)
